...
// Comment out below two lines for Xronos 3
...
void ht1632_plot (int x, int y, byte color)
{
  if (x<0 || x>=X_MAX || y<0 || y>=Y_MAX)
    return;
    // Stupid fix for bad 3rd matrix (do not use in production!)
    /*
    if (x>=24 && y < 8) {
...
boolean scrolltextsizexcolor(int y,char Str1[ ], byte color, int delaytime){
  ...
  while (xa<1) {
    ...
    while (xpos > (-1 * ( 8* messageLength))) {
      ...
      if ( (digitalRead(SET_BUTTON_PIN) == BUTTON_ON) || (digitalRead(MENU_BUTTON_PIN) == BUTTON_ON) || (digitalRead(INC_BUTTON_PIN) == BUTTON_ON)  ) {cls (); return false; } // Interrupt
...
/* =====================================================================================
 * Vertical Scrolling In/Out
 *  by LensDigital
 * Parameters: 
 *  x: Start position
 *  secondHalf: true = bottom, false = top
 *  isDropping: true = scroll from/to top , false = scroll from/to bottom 
 *  flyIn: true = Fly text in, false = fly text out of screen
 *  Str1: array of chars
 *  txtSize: 1-2: Normal, 3: tiny
 *  color: color
 *  delaytime: animation speed in ms
 * Please use only fonts with fixed heigth and width, otherwise the
 * chars will be overlapping
  ====================================================================================*/
boolean jumpTextVertical(int x,boolean secondHalf, boolean isDropping, boolean flyIn, char Str1[ ], byte txtSize, byte color, int delaytime){
   //boolean bounce=false;
   int ypos = Y_MAX; // By default get max
   int yEnd=0;// By default end up on top
   if (secondHalf) yEnd=8; // End up in the middle (second half of screen)
   int endLimit = yEnd; // Changes depending if we flying in or out
   
   if (isDropping) {  //scrolling from top to bottom
   // -------------------  
    if (flyIn) ypos = -Y_MAX; // We are flying in
    else {
      ypos = yEnd; // Flying out
      endLimit=Y_MAX;
    }
    for (ypos;ypos<=endLimit;ypos++) {
      showText(x,ypos,Str1, txtSize, color);
      delay (delaytime);
    }
   }
    // -------------------
   else {// Raising (scrolling from bottom to top
     if (!flyIn) {
        ypos = yEnd; // Flying out
        endLimit=-Y_MAX;
      }
   ...
   //if ( (digitalRead(SET_BUTTON_PIN) == LOW) || (digitalRead(MENU_BUTTON_PIN) == LOW) || (digitalRead(INC_BUTTON_PIN) == LOW)  ) {cls (); return false; } // Interrupt
   return true; // wasn't interrupted
}

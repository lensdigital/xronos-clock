...
void procAlarm(byte alrmnum) {
  if (isInMenu) return; // Do not sound alarm if changing settings
    //if (digitalRead(SET_BUTTON_PIN) == LOW) processSetButton(); // Poll Set Button, that will reArm (cancel for today) alarm
    if ( buttonsStatus[SET_BUTTON] == B_PRESSED ) processSetButton(); // If Set Button was pressed, snooze
...
void rearmAlrm(byte alrmnum){
  if (alarmon[alrmnum] & 128) { // Is global alarm switch on? (1st bit is set?)
    if ( ( myhours==alrmHH[alrmnum]+1) && ( minute()==alrmMM[alrmnum]) ) { // It's been 1 hour since Alarm sounded
...
//    wave.stop();
    ...
    //blinking=true; // Work aourn for bug when Pressing reset button stops blinking of the dots.
    if (alrmToneNum[alrmnum]<6) alrmVol[alrmnum]=7; //Set low volume for escalating alarms
    else alrmVol[alrmnum]=0; // Set High volume for non-escalating alarms
    return true;
    }

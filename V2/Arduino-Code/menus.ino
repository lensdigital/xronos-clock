...
char *sysMenuStr[11][2]={ {"Mode:",""},{"Light",""},{"Color",""}, {"Font",""}, {"Menu","sFX"},{"Temp",""},{"Sound","Vol"},{"Startup",""},{"RFM",""},{"IR",""}, {"RESET","FLASH"} };
...
char *optInfoDispStr[9]={"Scrol", "Every","Date", "InTemp", "ExTemp", "Alarm", "RFStat", "Humid","Exit" };
char *optSayStr[8]={"Say", "Time", "Date", "InTemp","ExTemp", "Humid", "Alrm", "Exit" };
...
void userMenu () {
     ...
     if (subMenu[USER_MENU]==1) { // Do not increment this submenu (we are in Infodisplay Options menu)
       ...
       if (subMenu[SCROLL_MENU] == 0) subMenu[USER_MENU]++; // Go to next item, we are not enterying this submenu tree
       else {
         subMenu[SCROLL_MENU]++;
         if (subMenu[SCROLL_MENU] > 8) subMenu[SCROLL_MENU]=1; // Goes back to first item of this submenu 
       }
     }
     else if (subMenu[USER_MENU]==2) { // Do not increment this submenu (we are in Talking Options menu)
              //putstring_nl ("In subMenu");
              
             if (subMenu[TALK_MENU] == 0) subMenu[USER_MENU]++; // Go to next item, we are not enterying this submenu tree
             else {
               subMenu[TALK_MENU]++;
               if (subMenu[TALK_MENU] > 7) subMenu[TALK_MENU]=1; // Goes back to first item of this submenu 
             }
     } 
...
void butSetClock() {
  cls();
        ...
        if (subMenu[TIME_MENU] > 7) subMenu[TIME_MENU]=1; // Goes back to first menu item
      	switch (subMenu[TIME_MENU]) {
          case 1: // Set hours

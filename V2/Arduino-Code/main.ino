...
void talkingLogic () {
    ...
    switch (mbutState){ // Skip item if it's disabled in EEProm
     case 1: 
     if (!(sayOptions & SO_Time)) {
       mbutState++;
       if (!(sayOptions & SO_Date)) {
         mbutState++; 
         if (!(sayOptions & SO_InTemp)) { 
           mbutState++;
           if (!(sayOptions & SO_OutTemp)) {
             mbutState++;
             if (!(sayOptions & SO_OutHum)) {
               mbutState++;
               if (!(sayOptions & SO_Alarms)) {
                 mbutState++;
                 return;
               }
             }
     ...
     break;
     case 6:
     if (!(sayOptions & SO_Alarms)) mbutState++;
     break;
    }
...
void showSensorData() {
  char myString[44];
   if (RFRecieved) { // Show sensor timestamp
          ...
          byte third=sBatt%10;
          showSmTime(0,clockColor); // Show Time on top
          snprintf(myString,sizeof(myString), "Recieved at %02d:%02d",hour(last_RF),minute(last_RF));
          ...
          showSmTime(0,clockColor); // Show Time on top
          snprintf(myString,sizeof(myString), "No Sensor Data in over %2d minutes",RF_TEMOUT);
